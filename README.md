...
- `--virt-type qemu` forces software emulation. KVM can accelerate a VM only when the guest architecture matches what the host CPU can run, so an `x86_64` host cannot KVM-accelerate an `aarch64` guest. If libvirt tried to use KVM here, VM creation or startup would fail; with `qemu`, QEMU uses CPU emulation instead.
- `--cpu cortex-a72` chooses the emulated ARM CPU model, but it does not choose the guest architecture. Without it, libvirt/QEMU may pick a version-dependent ARM CPU default. The VM may still boot, but the guest CPU features become less explicit and less reproducible.
...
- `loader.readonly=yes` says the firmware code image is immutable. This matters on NixOS because the firmware file lives in the read-only Nix store and because UEFI variables belong in the separate NVRAM file, not in the firmware code image. If this is omitted, libvirt may not mark the code image read-only, and QEMU can try to open firmware storage in a way that is either writable in the wrong place or rejected by the filesystem.
- `loader.type=pflash` says the firmware is exposed as persistent flash memory, which is how AAVMF/OVMF-style UEFI firmware is provided to QEMU guests. If omitted, libvirt does not emit the pflash loader type; that can prevent the loader and NVRAM image from being treated as the split UEFI flash devices this setup expects.
