...
The `~/github` directory holds clones of GitHub repositories. Anytime it would be useful to have the contents of some file or files from `github.com/$OWNER/REPO`, use a clone of that repo in `~/github/$OWNER/$REPO`. If the clone already exists, you may need to `git pull` if the commit you're interested in is more recent. If the clone does not already exist, simply create it and then use that. Since this VM doesn't have GitHub credentials, you need to clone via HTTPS:
